...
def _rows(data: dict, key: str, required: tuple[str, ...]) -> list[dict]:
    ...
    if (
        ...
        or len(rows) > 100_000
        or any(not isinstance(row, dict) or any(k not in row for k in required) for row in rows)
    ):
...
class OWCSKoreaAdapter:
    ...
    async def _asset(self, release: dict, pattern: str):
        ...
        matches = [
            a
            for a in assets
            if isinstance(a, dict) and re.fullmatch(pattern, str(a.get("name", "")))
        ]
        if len(matches) != 1:
            raise _error(
                "Release does not contain exactly one required JSON asset.",
                release.get("html_url", REPOSITORY),
            )
        asset = matches[0]
        ...
        if (
            ...
            or not parsed.path.startswith("/IanBosworth/owcs-korea-data/releases/download/")
            or parsed.query
            or parsed.fragment
        ):
        ...
        if type(asset.get("size")) is not int or not 0 < asset["size"] <= MAX_ASSET_BYTES:
            raise SourceError(
                "SOURCE_UNAVAILABLE",
                "OWCS asset exceeds the bounded JSON download limit or has no declared size.",
                self.name,
                url,
            )
        ...
        if (
            len(json.dumps(result.data, separators=(",", ":"), ensure_ascii=False).encode())
            > MAX_ASSET_BYTES
        ):
            raise SourceError(
                "SOURCE_UNAVAILABLE",
                "OWCS decoded JSON asset exceeds its size limit.",
                self.name,
                url,
            )
        return result
    ...
    async def fetch(self, filters: dict) -> SourceResult:
        ...
        if view not in self.capabilities or region not in ("KOREA", "KR") or extra:
            raise SourceError(
                "UNSUPPORTED_FILTER",
                "OWCS Korea supports esports views and documented dataset filters only.",
                self.name,
            )
        ...
        if (
            ...
            raise SourceError(
                "UNSUPPORTED_FILTER", "limit must be 1–500 and offset 0–100000.", self.name
            )
        for key in ("after", "before"):
            if filters.get(key):
                try:
                    ...
                    raise SourceError(
                        "UNSUPPORTED_FILTER", f"{key} must be an ISO calendar date.", self.name
                    ) from exc
        if filters.get("after") and filters.get("before") and filters["after"] > filters["before"]:
            raise SourceError(
                "UNSUPPORTED_FILTER", "after must not be later than before.", self.name
            )
        heroes = filters.get("heroes") or ([filters["hero"]] if filters.get("hero") else [])
        if not isinstance(heroes, list) or len(heroes) > 100:
            raise SourceError(
                "UNSUPPORTED_FILTER", "heroes must be a list with at most 100 entries.", self.name
            )
        ...
        if role and view != "hero_meta":
            raise SourceError(
                "UNSUPPORTED_FILTER", "role is supported only by hero_meta.", self.name
            )
        ...
        if (
            ...
            raise _error(
                "GitHub latest-release metadata is missing or not a public stable release.",
                LATEST_RELEASE,
            )
        ...
        if (
            ...
            or manifest.get("release_version") != tag
            or not isinstance(manifest.get("safety"), dict)
            or manifest["safety"].get("approved_maps_only") is not True
        ):
            raise _error(
                "OWCS manifest schema, release identity, or approved-only guarantee changed.",
                manifest_response.url,
            )
        dataset_response = await self._asset(
            release, r"owcs-korea-data-" + re.escape(tag) + r"\.json"
        )
        dataset = dataset_response.data
        if dataset.get("schema_version") != SCHEMA_VERSION or dataset.get("release_version") != tag:
            raise _error(
                "OWCS normalized dataset schema or release identity changed.", dataset_response.url
            )
        source_matches = _rows(
            dataset, "matches", ("id", "match_date", "team_a", "team_b", "stage")
        )
        ...
        source_segments = _rows(
            dataset,
            "hero_time_segments",
            ("map_id", "hero", "team", "duration_seconds", "echo_copy_target"),
        )
        ...
        for key, rows in (
            ...
            ("maps", source_maps),
            ("hero_bans", source_bans),
            ("hero_time_segments", source_segments),
        ):
            if counts.get(key) != len(rows):
                raise _error(
                    f"OWCS {key} count differs from its release manifest.", dataset_response.url
                )
        ...
        maps = [
            ...
            and (not map_filter or _key(row["map_name"]) == _key(map_filter))
            and (not filters.get("map_id") or str(row["id"]) == str(filters["map_id"]))
        ]
        ...
        segments = [
            ...
            and (not filters.get("team") or _key(row["team"]) == _key(filters["team"]))
        ]
        if hero_set:
            if view == "bans":
                selected_map_ids = {
                    row["map_id"] for row in source_bans if normalize_hero(row["hero"]) in hero_set
                }
            else:
                selected_map_ids = {
                    row["map_id"]
                    for row in segments
                    if row["hero"].lower() != "unknown" and normalize_hero(row["hero"]) in hero_set
                }
        ...
        warnings = [
            "OWCS Korea is an independent, approved-map esports dataset, not competitive ladder statistics or evidence of a KR match server."
        ]
        responses = [release_response, manifest_response, dataset_response]
        scoped = any(
            filters.get(k) not in (None, "", "all")
            for k in ("map_id", "match_id", "team", "stage", "after", "before")
        ) or bool(map_filter)
        if view == "hero_meta" and not scoped:
            stats_response = await self._asset(
                release, r"owcs-korea-hero-stats-" + re.escape(tag) + r"\.json"
            )
            responses.append(stats_response)
            stats = stats_response.data
            if (
                stats.get("metric_version") != manifest.get("methodology_version")
                or not isinstance(stats.get("coverage"), dict)
                or stats["coverage"].get("approved_only") is not True
                or not isinstance(stats.get("export_manifest"), dict)
                or stats["export_manifest"].get("data_as_of") != data_until
            ):
                raise _error(
                    "OWCS aggregate statistics disagree with the release manifest.",
                    stats_response.url,
                )
            records = []
            for row in _rows(
                stats,
                "hero_stats",
                ("hero", "role", "hero_seconds", "pick_rate", "unmirrored_winrate"),
            ):
                ...
                records.append(
                    {
                        **row,
                        "hero": hero,
                        "hero_name": row["hero"],
                        "metric_origin": "published_release_aggregate",
                        "rate_unit": "ratio",
                    }
                )
            warnings.append(
                "Published pick_rate is ban-adjusted hero-time opportunity; unmirrored_winrate uses unmirrored time. Neither is competitive-match win rate."
            )
        elif view == "hero_meta":
            if role:
                raise SourceError(
                    "UNSUPPORTED_FILTER",
                    "Scoped hero-time queries cannot apply role without a verified per-segment role field.",
                    self.name,
                    dataset_response.url,
                )
            records = self._hero_time(segments, map_by_id, hero_set)
            warnings.append(
                "Scoped hero_seconds sum published segments before aggregate overlap removal. Rates remain null because the published aggregate denominators do not apply to this subset."
            )
            warnings.append(
                "Echo copy-target seconds remain part of Echo playtime and are listed separately; copied heroes are not counted as ordinary hero picks."
            )
        elif view == "matches":
            records = [
                {**row, "maps_included": sum(m["match_id"] == row["id"] for m in maps)}
                for row in matches
            ]
            ...
            records = [
                {
                    **row,
                    "map": normalize_map(row["map_name"]),
                    "match_date": match_by_id[row["match_id"]]["match_date"],
                }
                for row in maps
            ]
        elif view == "bans":
            records = [
                {
                    **row,
                    "hero": normalize_hero(row["hero"]),
                    "hero_name": row["hero"],
                    "map_name": map_by_id[row["map_id"]]["map_name"],
                }
                for row in source_bans
                if row["map_id"] in map_ids
                and (not hero_set or normalize_hero(row["hero"]) in hero_set)
                and (
                    not filters.get("team") or _key(row["banned_by_team"]) == _key(filters["team"])
                )
            ]
        else:
            ...
            records = [
                {
                    "team": name,
                    "matches_included": sum(
                        name in (row["team_a"], row["team_b"]) for row in matches
                    ),
                    "maps_won": sum(row["winner_team"] == name for row in maps),
                    "hero_seconds": round(
                        sum(
                            row["duration_seconds"]
                            for row in segments
                            if row["team"] == name
                            and row["hero"].lower() != "unknown"
                            and (not hero_set or normalize_hero(row["hero"]) in hero_set)
                        ),
                        6,
                    ),
                    "metric_origin": "sum_published_segments_before_overlap_removal",
                }
                for name in names
                if not filters.get("team") or _key(name) == _key(filters["team"])
            ]
        total = len(records)
        ...
        if not selected:
            warnings.append(
                "EMPTY_RESULT: the release was read successfully but no records match this page of filters."
            )
        self.latest_release = tag
        return SourceResult(
            data={
                "dataset_type": "esports",
                "release_tag": tag,
                "published_at": release["published_at"],
                "data_until": data_until,
                "coverage_period": full_period,
                "license": manifest.get("license"),
                "attribution": "OWCS Korea Meta data project / IanBosworth",
                "repository_url": REPOSITORY,
                "methodology": manifest.get("methodology_version"),
                "approved_maps_only": True,
                "view": view,
                "total": total,
                "offset": offset,
                "limit": limit,
            ...
            source=self.name,
            source_url=release.get("html_url", REPOSITORY),
            retrieved_at=min(r.retrieved_at for r in responses),
            source_updated_at=manifest.get("generated_at_utc"),
            data_period=full_period,
            ...
            stale=any(r.stale for r in responses),
        )

    @staticmethod
    def _match_matches(row: dict, filters: dict) -> bool:
        return (
            (not filters.get("match_id") or str(row["id"]) == str(filters["match_id"]))
            and (
                not filters.get("team")
                or _key(filters["team"]) in (_key(row["team_a"]), _key(row["team_b"]))
            )
            and (
                filters.get("stage") in (None, "", "all")
                or _key(row["stage"]) == _key(filters["stage"])
            )
            and (not filters.get("after") or row["match_date"] >= filters["after"])
            and (not filters.get("before") or row["match_date"] <= filters["before"])
        )
    ...
    def _validate_relations(matches, maps, bans, segments):
        try:
            for rows, text_fields in (
                (matches, ("match_date", "team_a", "team_b", "stage")),
                (maps, ("map_name",)),
                (bans, ("hero", "banned_by_team")),
                (segments, ("hero", "team")),
            ):
                if any(
            ...
            if (
                ...
                or len(map_ids) != len(maps)
                or any(row["match_id"] not in match_ids for row in maps)
                or any(row["map_id"] not in map_ids for row in bans + segments)
            ):
    ...
    def _hero_time(segments, maps, hero_set):
        ...
        for segment in segments:
            ...
            group = groups.setdefault(
                ...
                {
                    "hero": hero,
                    "hero_name": segment["hero"],
                    "hero_seconds": 0.0,
                    "map_ids": set(),
                    "match_ids": set(),
                    "teams": set(),
                    "by_map": defaultdict(float),
                    "echo_copy_targets": defaultdict(float),
                },
            )
        ...
        for group in groups.values():
            records.append(
                {
                    "hero": group["hero"],
                    "hero_name": group["hero_name"],
                    "hero_seconds": round(group["hero_seconds"], 6),
                    "maps_appeared": len(group["map_ids"]),
                    "matches_appeared": len(group["match_ids"]),
                    "teams": sorted(group["teams"]),
                    "by_map": [
                        {"map_name": key, "hero_seconds": round(value, 6)}
                        for key, value in sorted(group["by_map"].items())
                    ],
                    "echo_copy_targets": [
                        {"hero": key, "seconds": round(value, 6)}
                        for key, value in sorted(group["echo_copy_targets"].items())
                    ],
                    "pick_rate": None,
                    "unmirrored_winrate": None,
                    "metric_origin": "sum_published_segments_before_overlap_removal",
                }
            )
