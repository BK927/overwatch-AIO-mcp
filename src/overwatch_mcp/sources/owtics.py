...
def decode_loader(html: str, route: str, url: str) -> dict[str, Any]:
    ...
    for script in soup.find_all("script"):
        ...
        try:
            ...
            route_index = next(
                v for k, v in values[loader_index].items() if values[int(k[1:])] == route
            )
...
class OwticsAdapter:
    ...
    async def meta(self, filters: dict) -> SourceResult:
        ...
        if (
            ...
            or mode not in ("COMPETITIVE", "QUICKPLAY")
        ):
            raise SourceError(
                "UNSUPPORTED_FILTER",
                "Unsupported OWTICS filter; Grandmaster and Champion are available only as a combined tier.",
                self.name,
            )
        if platform and str(platform).lower() != "pc":
            raise SourceError(
                "UNSUPPORTED_FILTER", "OWTICS has no verified console/platform selector.", self.name
            )
        if mode == "QUICKPLAY" and (map_slug != "all-maps" or tier != "ALL"):
            raise SourceError(
                "UNSUPPORTED_FILTER",
                "OWTICS quickplay is verified only for overall, all-tier statistics.",
                self.name,
            )
        ...
        if not isinstance(heroes, list) or len(heroes) > 100:
            raise SourceError(
                "UNSUPPORTED_FILTER", "heroes must be a list of at most 100 identifiers.", self.name
            )
        hero_set = {normalize_hero(hero) for hero in heroes}
        path = "/hero" if map_slug == "all-maps" else "/map/" + quote(map_slug, safe="-")
        response = await self.client.get_text(
            self.name,
            BASE_URL + path,
            params={"region": region, "tier": tier, "mode": mode},
            ttl=3600,
        )
        url = response.url
        route = decode_loader(
            response.data,
            "app/routes/hero" if map_slug == "all-maps" else "app/routes/map.$slug",
            url,
        )
        warnings = [
            "OWTICS source-region labels describe its own population; they do not verify player country or match server location."
        ]
        if platform:
            warnings.append(
                "Requested PC platform cannot be verified from OWTICS' payload; no platform filter is claimed as applied."
            )
        try:
            if map_slug == "all-maps":
                ...
                if any(
                    effective.get(k) != value
                    for k, value in {"region": region, "tier": tier, "mode": mode}.items()
                ):
                    raise SourceError(
                        "UNSUPPORTED_FILTER",
                        "OWTICS did not apply the requested region, tier, or mode.",
                        self.name,
                        url,
                    )
                ...
                if any(
                    ...
                    raise _parse_error(
                        "OWTICS overview contains mismatched measurement filters.", url
                    )
            else:
                ...
                if map_data["slug"] != map_slug:
                    raise SourceError(
                        "UNSUPPORTED_FILTER",
                        "OWTICS resolved a different map than requested.",
                        self.name,
                        url,
                    )
                ...
                if effective.get("tier") != tier or route.get("region") != region:
                    raise SourceError(
                        "UNSUPPORTED_FILTER",
                        "OWTICS did not apply the requested map tier or region.",
                        self.name,
                        url,
                    )
                ...
                if not any(
                    ...
                    and c.get("tier") == tier
                    and c.get("mode") == mode
                    and c.get("season", {}).get("id") == effective.get("season", {}).get("id")
                    and "BY_MAP" in c.get("grains", [])
                    for c in combos
                ):
                    raise SourceError(
                        "UNSUPPORTED_FILTER",
                        "OWTICS catalog does not confirm this map filter combination.",
                        self.name,
                        url,
                    )
                all_rows = container["result"]["measurements"]
                if not isinstance(all_rows, list) or any(
                    not isinstance(row, dict) or "region" not in row for row in all_rows
                ):
            ...
            for row in rows:
                ...
                if map_slug == "all-maps" and row.get("season", {}).get("id") != season["id"]:
                    raise _parse_error(
                        "OWTICS measurement season does not match its applied filter.", url
                    )
        ...
        if filters.get("order_by"):
            ...
            if (
                len(ordering) != 2
                or ordering[0] not in ("pickrate", "winrate", "banrate")
                or ordering[1] not in ("asc", "desc")
            ):
                raise SourceError(
                    "UNSUPPORTED_FILTER",
                    "order_by must be a rate field followed by :asc or :desc.",
                    self.name,
                )
            ...
            normalized = (
                sorted(present, key=lambda row: row[metric], reverse=direction == "desc") + absent
            )
