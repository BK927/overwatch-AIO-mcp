...
CATALOG = {
    ...
    "tiers": [
        ...
        {"ID": 31, "tier": "Grandmaster", "tierId": "grandmaster-5", "title": "Grandmaster 5", "division": "5"},
        {"ID": 35, "tier": "Grandmaster", "tierId": "grandmaster-1", "title": "Grandmaster 1", "division": "1"},
        {"ID": 40, "tier": "Champion", "tierId": "champion-1", "title": "Champion 1", "division": "1"},
    ],
}
REPLAY = {
    "Code": "8D8VRC", "Heroes": [19], "Map": 34, "Tier": 35,
    "Player": "WHORU", "UserHandle": "WHORU", "Verified": True,
    "Archived": True, "CodeVerificationStatus": "valid", "Uploaded": 1777611129,
    "Platform": "pc", "PatchLevel": "2.22.0.1",
}
...
class FakeClient:
    ...
    async def get_json(self, source, url, params=None, ttl=0):
        ...
        if url.endswith("replays"):
            return self.result({"replays": self.replay if isinstance(self.replay, list) else [self.replay], "pages": self.pages}, url)
...
def test_search_resolves_catalog_filters_and_expands_minimum_tier():
    client = FakeClient()
    result = asyncio.run(OWReplaysAdapter(client).search({"hero": "reinhardt", "tier_min": "GRANDMASTER", "platform": "PC"}))
...
@pytest.mark.parametrize("field,value", [("match_region", "KR"), ("country", "KR"), ("player_pool", "kr_rankers")])
...
def test_search_is_bounded_and_never_promotes_working_state():
    ...
    result = asyncio.run(OWReplaysAdapter(client).search({"max_pages": 2, "playable_status": ["client_verified"]}))
    assert result.data == []
...
@pytest.mark.parametrize("body,is_empty", [({"error": "Invalid replay"}, True), ({"error": "Access denied"}, False)])
...
def test_patch_hero_filter_preserves_replay_notice_and_does_not_invent_version():
    records = PatchAdapter(None).parse(PATCH, "https://overwatch.blizzard.com/en-us/news/patch-body/live/2026/8", "reinhardt")
    assert len(records) == 1
    record = records[0]
    assert record["hero_changes"] == [{"hero": "reinhardt", "description": "Barrier Field health increased.", "context_headings": []}]
...
def test_patch_accented_name_normalizes_and_nonmatching_hero_is_empty():
    ...
    assert adapter.parse(PATCH, "https://overwatch.blizzard.com/en-us/news/patch-notes/", "lucio")[0]["hero"] == "lucio"
    assert adapter.parse(PATCH, "https://overwatch.blizzard.com/en-us/news/patch-notes/", "ana") == []


@pytest.mark.parametrize("notice,effect", [
...
def test_only_explicit_replay_compatibility_changes_are_classified(notice, effect):
    ...
    record = PatchAdapter(None).parse(html, "https://overwatch.blizzard.com/en-us/news/patch-notes/")[0]
...
def test_archive_months_are_bounded_and_coverage_is_reported():
    ...
    result = asyncio.run(PatchAdapter(client).fetch({"before": "2026-08-31", "after": "2025-01-01", "max_months": 2}))
...
def test_patch_wrong_month_or_missing_container_is_not_empty():
    with pytest.raises(SourceError) as error:
        asyncio.run(PatchAdapter(FakeClient(html=PATCH)).fetch({"before": "2026-07-31", "max_months": 1}))
    assert error.value.code == "PARSE_ERROR"
...
def test_source_empty_month_marker_is_distinct_from_changed_structure():
    adapter = PatchAdapter(None)
    assert adapter.parse('<div class="PatchNotes-body"><p class="patch-notes-error">No Patch Notes Found</p></div>', "https://overwatch.blizzard.com") == []
    with pytest.raises(SourceError):
        adapter.parse('<div class="PatchNotes-body"><article>New patch markup</article></div>', "https://overwatch.blizzard.com")


def test_hero_change_preserves_stadium_section_context():
    html = PATCH.replace('<div class="PatchNotesHeroUpdate">', '<h4 class="PatchNotes-sectionTitle">Stadium Updates</h4><h4 class="PatchNotes-sectionTitle">Tank</h4><div class="PatchNotesHeroUpdate">', 1)
