...
> 기존 `v0.3.1` MCPB에는 원래 표시명 `Overwatch AIO MCP`가 남아 있습니다.
> `0.3.2`부터는 새 표시명 `Overwatch 2 Data MCP Server`를 사용합니다.
> 이미 공개한 릴리스 파일은 덮어쓰지 않습니다.

**오버워치 2 영웅 메타, 공개 플레이어 통계, 근거가 등록된 랭커, 리플레이 코드, 공식 패치 노트, OWCS Korea 자료**를 MCP 클라이언트나 독립 실행형 에이전트 스킬에서 조회합니다. 두 방식은 읽기 조회 10개와 SQLite 저장소를 공유하며, 결과에 출처 URL·관측 날짜·적용 필터·캐시 상태·경고를 보존합니다.

지역·티어별 영웅 비교, 공개 BattleTag 프로필, 검증 근거가 있는 랭커·리플레이 검색, 패치 변경과 실제 자료 기간의 연결에 사용할 수 있습니다. 경쟁전 통계와 e스포츠 지표는 섞지 않습니다. 데이터는 요청할 때만 가져오며 정기 수집, 게임 클라이언트 자동화, 비공개 프로필 접근은 제공하지 않습니다.

Python 3.11 이상과 `uv`를 사용합니다. 스킬, MCP 서버 또는 둘 다 선택할 수 있습니다.
...
| 환경 | 상태 | 요구 사항과 제한 |
|---|---|---|
| 로컬 데스크톱, stdio | 지원 | 기본 구성입니다. MCP 클라이언트가 프로세스를 시작하고 사용자별 SQLite DB를 로컬에 보존합니다. |
| 로컬 데스크톱, Streamable HTTP | 지원 | 인증된 게이트웨이를 이미 구성한 경우가 아니면 기본 loopback 주소를 유지합니다. 서버 자체 인증은 없습니다. |
| 개인 홈 서버 | 운영자 구성에서만 가능 | 서비스 관리자, 영속 SQLite DB, 백업, 신뢰할 수 있는 리버스 프록시 또는 사설망 경계의 TLS·인증이 필요합니다. MCP 원본 포트를 직접 공개하지 않습니다. |
| Raspberry Pi 4 Model B (2GB RAM) | 테스트 장비일 뿐 | 실제 테스트에 사용한 장비를 기록한 것이며 권장 사양·최소 요구사항·성능 보장이 아닙니다. |
| Google Compute Engine VM | 수동 구성, 미검증 | 영속 디스크를 연결한 단일 VM에서 실행할 수 있지만 이미지나 Terraform 모듈은 없습니다. TLS·인증, 백업, 단일 DB 쓰기 소유권, 실제 외부 출처 연결을 검증해야 합니다. |
| Cloudflare Tunnel | 운영자 수동 ingress 가능, 미검증 | 인증을 설계·검증한 뒤 홈 서버나 VM 앞에 둘 수 있습니다. 실행 중인 호스트로 연결하는 ingress일 뿐 Cloudflare Workers 런타임이 아닙니다. |
| Google Cloud Run | 현재 구조로 미지원 | 로컬 SQLite는 인스턴스 로컬이며 stateless 컨테이너·외부 저장소 구성이 없습니다. 영속 저장소, 인증, 수명주기 설계를 바꿔야 합니다. |
| Cloudflare Workers | 현재 구조로 미지원 | 현재 CPython MCP SDK와 로컬 SQLite 구조는 Worker 배포가 아닙니다. stateless HTTP 처리, Worker 네이티브 저장소, 호환 인증으로 포팅해야 합니다. |

원격·홈 서버 사용은 내장 호스팅 기능이 아니라 운영자가 구성하는 배포 패턴입니다. 비-loopback 주소로 바인딩하려면 모든 요청이 운영자 소유 TLS·인증을 먼저 거쳐야 하고 DB가 영속 저장소에 있어야 합니다.
...
아닙니다. `ASIA`는 제공자의 지역 라벨이며 한국 경기 서버의 증거가 아닙니다. OWTICS의 `KOREA`도 제공자 분류입니다. 결과에서는 이 라벨과 근거가 등록된 플레이어 신원을 구분합니다.

### 실시간 Top 500 전체 목록을 제공하나요?

아닙니다. `ow_rankers_search`는 로컬에서 근거를 등록한 명부만 검색합니다. 전체 실시간 Top 500을 확보했다고 주장하지 않습니다.

### 반환된 리플레이 코드는 반드시 재생되나요?

아닙니다. 공개 코드만으로 현재 재생 가능성을 증명할 수 없습니다. 운영자가 근거를 등록한 경우에만 재생 상태를 표시하며 이 프로젝트는 오버워치 2를 실행하거나 제어하지 않습니다.

### 계속 자동 수집하거나 완전히 오프라인으로 동작하나요?

스케줄러는 없습니다. 새로고침 조회는 요청할 때 공개 출처에 접속합니다. 저장된 관측·수동 근거와 각 조회가 허용하는 캐시/상태 결과는 오프라인에서도 사용할 수 있지만, 오프라인 상태에서 자료가 최신으로 갱신되지는 않습니다.

### 비공개 프로필을 읽거나 MCP를 바로 공개할 수 있나요?

비공개 프로필을 우회하지 않습니다. 공개 호스팅도 즉시 사용할 수 있는 기능이 아닙니다. 현재 HTTP 서버에는 자체 인증이 없으므로 운영자가 TLS, 인증, 영속 저장소, 접근 제어, 백업을 제공해야 합니다.
...
이 프로젝트는 비공식 커뮤니티 프로젝트이며 Blizzard Entertainment와 제휴하거나 보증받지 않았습니다. Overwatch 및 관련 표장은 각 권리자에게 속합니다.
