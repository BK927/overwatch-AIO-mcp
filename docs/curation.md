...
MCP 조회는 읽기 전용이다. 근거를 만드는 명령은 운영자의 로컬 CLI에만 있다. 초기 명부 `examples/rankers.empty.json`은 빈 배열이며, 예시 선수나 국적을 실제 DB에 넣지 않는다.

## 명부 입력

`uv run overwatch-aio-mcp import-rankers my-rankers.json`
...
```text
uv run overwatch-aio-mcp link-replay ABC123 --player-id 1 --evidence-file replay-evidence.json
```
...
`uv run overwatch-aio-mcp discover-rankers`는 저장된 고티어 공개 리플레이의 작성자를 검토 후보로 보여 준다. 후보의 국적과 Top 500 여부는 미확인이고 명부에 자동 등록되지 않는다.
...
```text
uv run overwatch-aio-mcp record-replay-check ABC123 --status client_verified --source "실제 확인자" --evidence-ref "검증 기록 또는 캡처의 위치" --observed-at "실제 확인 시각(ISO 8601)"
```

직접 클라이언트 확인 증거 없이 사용자 제보만 받은 경우 `user_reported_working`을 사용한다. MCP나 웹 소스는 `client_verified`를 자동 생성하지 않는다. 이 명령은 게임을 실행하거나 코드를 검사하는 명령이 아니다.

공식 패치에 명확한 replay invalidation 공지가 있으면 이전 검증이 `needs_recheck`로 바뀐다. 새 패치 이후의 실제 확인은 다시 기록할 수 있다. 원본 웹 출처가 expired라고 한 사실과 로컬 검증 이력은 모두 보존한다.
